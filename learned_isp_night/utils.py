...
def process_command_args(arguments):
    ...
    # --- data path ---
    dataset_dir = '../raw_images_night/crops_night/'
    model_dir = 'learning_R_5e-05_RI_0/'
    result_dir = 'results/'
    ...
    restore_iter = 0
    # --- input size ---
    patch_w = 256  # default size for SKOL dataset
    patch_h = 256  # default size for SKOL dataset
    # --- training options ---
    batch_size = 32
    train_size = 5000
    learning_rate = 5e-5
    ...
    for args in arguments:
        ...
        if args.startswith("num_input_images"):
            num_input_images = int(args.split("=")[1])
    ...
    print("num_input_images: " + str(num_input_images))
...
def process_test_model_args(arguments):
    ...
    # --- data path ---
    dataset_dir = '../raw_images_night/'
    test_dir = 'crops_night/test/'
    model_dir = 'models/punet/learning_R_5e-05_RI_0_NI_3/'
    result_dir = 'results/full_res_test_results/'
    # --- architecture ---
    arch = "punet"
    level = 0
    inst_norm = False
    ...
    return dataset_dir, test_dir, model_dir, result_dir, \
           ...
           img_h, img_w, use_gpu, save_model, test_image, num_input_images


def process_test_scores_args(arguments):
    model_dir = "learning_R_0.0001_RI_0_NI_1/"
    num_input_images = 1
    ...
    for args in arguments:
        ...
        if args.startswith("model_dir"):
            model_dir = args.split("=")[1]
            num_input_images = int(model_dir.split("_")[-1].split("/")[0])

        if args.startswith("num_input_images"):
            num_input_images_temp = int(args.split("=")[1])
            if num_input_images_temp != num_input_images:
                print("Mismatch between manually entered number of input images: ", num_input_images_temp,
                      "and model-directory:", num_input_images)
            else:
                num_input_images = num_input_images_temp


    return model_dir, num_input_images
