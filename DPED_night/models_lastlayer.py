...
def resnet(input_image):
    with tf.compat.v1.variable_scope("generator"):
        W1 = weight_variable([9, 9, 4, 64], name="W1")
        ...
        W12 = weight_variable([9, 9, 64, 16], name="W12")
        b12 = bias_variable([16], name="b12")
        c12 = tf.nn.relu(conv2d(c11, W12) + b12)

        # Upscaling,  Input size: 128 x 128  Output size = 256 x 256
        c13 = _conv_tranpose_layer(c12, 4, 3, 2)  # 128 -> 256
        W14 = weight_variable([3, 3, 4, 3], name="W14")
        b14 = bias_variable([3], name="b14")

        enhanced = tf.nn.tanh(conv2d(c13, W14) + b14) * 0.58 + 0.5
...
def _conv_layer(net, num_filters, filter_size, strides, batch_nn=True, relu=True):
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides):
    ...
    if batch_size is None:
        new_shape = [1, new_rows, new_cols, num_filters]
    else:
