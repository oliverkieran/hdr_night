...
def process_command_args(arguments):
    ...
    for args in arguments:
        ...
        if args.startswith("from_model"):
            from_model = args.split("=")[1]

        if args.startswith("eval_step"):
            eval_step = int(args.split("=")[1])

        if args.startswith("input_images"):
            input_images = int(args.split("=")[1])

    # Read and write to same model_dir
    ...
    print("Run Id:", run_id)
    print("From_model:", from_model)
    print("Input Images:", input_images)
    print()
    return batch_size, train_size, learning_rate, num_train_iters, restore_iter, \
            loss, dped_dir, vgg_dir, run_id, from_model, eval_step, input_images


def process_test_model_args(arguments):

    dped_dir = "../../crops/"
    test_dir = "mediatek_raw/"
    iteration = "20000"
    resolution = "phone"  # [phone, full_res]
    use_gpu = "true"
    run_id = "default"
    input_images = 1
    ...
    return dped_dir, test_dir, iteration, resolution, use_gpu, run_id, input_images
