...
with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
    ...
    # final loss
    if loss == "pynet":
        loss_generator = 20 * loss_content + 15 * loss_mse + 1 * (1 - loss_ssim)
    else:
    ...
    merged = tf.compat.v1.summary.merge(
        [loss_generator_summ, loss_content_summ, loss_color_summ, loss_tv_summ, loss_psnr_summ, loss_ms_ssim_summ]
    )
    """
    merged_val = tf.compat.v1.summary.merge(
        [loss_val_generator, loss_content_summ, loss_color_summ, loss_tv_summ, loss_psnr_summ]
    )
    """
        ...
        # evaluate on validation data
            ...
            logs_gen = "step %d generator losses | train: %.4g, val: %.4g | content: %.4g, color: %.4g, tv: %.4g | psnr: %.4g, ms-ssim: %.4g\n" % \
                       (i, train_loss_gen, val_losses_gen[0][0], val_losses_gen[0][1], val_losses_gen[0][2],
                        val_losses_gen[0][3], val_losses_gen[0][4], val_losses_gen[0][5])  # loss_ssim)
            # logs_gen = "generator losses | train: %.4g, test: %.4g | content: %.4g, color: %.4g, texture: %.4g, tv: %.4g | psnr: %.4g, ms-ssim: %.4g\n" % \
            # (train_loss_gen, val_losses_gen[0][0], val_losses_gen[0][1], val_losses_gen[0][2],
            # val_losses_gen[0][3], val_losses_gen[0][4], val_losses_gen[0][5], loss_ssim)
