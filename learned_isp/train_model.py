...
with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
    ...
    # Final loss function
    loss_generator = loss_mse * 20 + (1 - loss_ssim) * 20 + loss_content
    ...
    if not os.path.exists("models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/"): os.makedirs("models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/")
    logs = open("models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/logs_" + str(iter_start) + "-" + str(iter_start + num_train_iters) + ".txt", "w+")
    logs.close()
    ...
    tensorboard_folder = './graphs/LR_' + str(learning_rate) + '_RI_' + str(restore_iter)
    ...
    for i in range(iter_start, iter_start + num_train_iters + 1):
        ...
        if i % eval_step == 0:
            ...
            logs = open("models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/logs_" + str(iter_start) + "-" + str(iter_start + num_train_iters) + ".txt", "a")
            logs.write(logs_gen)
            logs.write('\n')
            logs.close()
            ...
            print("saving model: " + "models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/" + name_model_save_full + ".ckpt")
            saver.save(sess, "models/punet/" + "learning_R_" + str(learning_rate) + '_RI_' + str(restore_iter) + "/" + name_model_save_full + ".ckpt", write_meta_graph=False)
